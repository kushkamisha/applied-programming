...
class Square:
  ...
  def update_neighbors(self, grid):
    ...
    if self.row < self.total_rows - 1 and \
            not grid[self.row + 1][self.col].is_barrier():  # RIGHT
    ...
    if self.col < self.total_rows - 1 and \
            not grid[self.row][self.col + 1].is_barrier():  # DOWN
      self.neighbors.append(grid[self.row][self.col + 1])
  ...
  while True:
    ...
    for event in pygame.event.get():
      ...
      if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_SPACE and start and end and not completed:
          for row in grid:
            for square in row:
