...
Технічні вимоги.
    Вхід   Текстовий файл, у першому рядку - кількість n цілих чисел
           (2 < n < 108), у наступних n рядках - цілі числа.
...
def iterate_file(_filename):
    greatest = []
    smallest = []
    a_positive = None

    with open("input/{}".format(_filename), "r") as f:
...
def display_answer(answer):
    print("\nЧисла, що дають максимальний добуток: {}, {} та {}"
        .format(*answer))
    print("Максимальним добутком є {}\n".format(math.prod(answer)))


def greatest_product(_filename):
    greatest, smallest, a_positive = iterate_file(_filename)
